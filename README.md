...
# Flowise, actually open source — `3.1.4-fw9`

```bash
docker pull dblagbro/flow-wiser:3.1.4-fw9     # or :latest
```
...
**`3.1.4-fw9` is the baseline.** It is the first release whose commit passes CI — fw5, fw6
...
**[BASELINE-3.1.4-fw9.md](docs/BASELINE-3.1.4-fw9.md)** — what is verified, how, and what is
...
📄 **[Baseline & QA scope](docs/BASELINE-3.1.4-fw9.md)** · **[CHANGELOG](CHANGELOG.md)** ·
...
```bash
docker pull dblagbro/flow-wiser:3.1.4-fw9     # or :latest
curl -s http://localhost:3000/api/v1/version  # {"version":"3.1.4-fw9"}
```

```
flowise=3.1.4-fw9   flowise-components=3.1.4-fw9   flowise-ui=3.1.4-fw9   vm2=3.11.5
```
...
docker build --no-cache --pull \
  ...
  --build-arg FLOWISE_VERSION=3.1.4-fw9 \
  -t dblagbro/flow-wiser:3.1.4-fw9 .
```
...
⚠️ **Licensing — `3.1.4-fw1` through `3.1.4-fw3` only.** Like every Flowise container published before 2026-08-06, those images contain compiled output from `packages/server/src/enterprise/` and `IdentityManager.ts`, which are under FlowiseAI's **Commercial License**, not Apache 2.0, and their terms govern those components wherever you obtain them. Flow-Wiser could not and did not relicense them. **`3.1.4-fw9` is clean**: the repository no longer contains those files, the release image is built from source, and the build fails if any trace of them reaches it. See [FORK.md](FORK.md).
