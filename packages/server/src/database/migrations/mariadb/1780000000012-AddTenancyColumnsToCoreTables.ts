 ...
 *   here: only the SQLite 1755066758601 rebuilds that table with the column, the MariaDB one only
 *   widens `type`.
 *
 ...
 *
 * IDEMPOTENCE. These indexes go onto tables that already exist, so they cannot be declared inline
 * as a `KEY` in a CREATE TABLE the way 1780000000000 does. MariaDB does accept
 * `CREATE INDEX IF NOT EXISTS`, but its MySQL counterpart does not, and the two files are kept
 * parallel: both directions are guarded with `hasIndex` (utils/database.util.ts), the companion to
 * the `hasColumn` guard used on the columns.
 *
 ...
 * counterpart of this migration has no MariaDB equivalent: 1755066758601 only emits it on SQLite.)
 */
