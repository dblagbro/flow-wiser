...
export class AddIdentityMfaAuditTables1780000000002 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const sessionColumns: [string, string][] = [
            ...
            ['loginMethodId', 'varchar'],
            ['authProvider', 'varchar(20)'],
            ['mfaSatisfied', 'boolean NOT NULL DEFAULT 0'],
            ['mfaFactorId', 'varchar'],
            ['mfaSatisfiedDate', 'datetime'],
        ...
        if (!(await hasColumn(queryRunner, 'identity_role', 'requiresMfa'))) {
            await queryRunner.query(`ALTER TABLE "identity_role" ADD COLUMN "requiresMfa" boolean NOT NULL DEFAULT 0;`)
        }
        ...
        // invert them). SQLite gives a gap-free-until-tampered sequence only for the exact form
        // `INTEGER PRIMARY KEY AUTOINCREMENT`, which is why the column is `integer` here and
        // `bigserial` / `bigint AUTO_INCREMENT` on the other engines. `id` stays a uuid so external
        // references (SIEM export, cross-links) do not leak event volume.
        await queryRunner.query(`
            CREATE TABLE IF NOT EXISTS "identity_audit_event" (
                "seqNo" integer PRIMARY KEY AUTOINCREMENT NOT NULL,
                "id" varchar NOT NULL,
        ...
        await queryRunner.query(
            `CREATE INDEX IF NOT EXISTS "IDX_identity_audit_event_occurredAt" ON "identity_audit_event" ("occurredAt");`
        )
        ...
        await queryRunner.query(`
            CREATE VIEW IF NOT EXISTS "identity_login_activity" AS
            SELECT
                "id" AS "id",
                "seqNo" AS "seqNo",
