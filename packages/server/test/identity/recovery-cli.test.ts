 ...
 *
 * ── Running them ─────────────────────────────────────────────────────────────────────────────
 * `packages/server/jest.config.js` currently sets `roots: ['<rootDir>/src']` and maps `typeorm` to a
 * decorator mock, so `pnpm test` picks up neither this file nor the existing `totp-rfc6238.test.ts`
 * next to it, and could not use a real TypeORM if it did. Until that config grows a second project,
 * run them explicitly from `packages/server`:
 *
 *     npx jest --rootDir . --roots test/identity --testRegex '.*\.test\.ts$' \
...
describe('admin:create', () => {
    ...
    it('refuses a password on the published-example denylist', async () => {
        await expect(
            createAdminAccount({ ...context(), email: 'denied@example.com', role: 'admin', password: 'Flowise!1' })
...
describe('admin:reset-password', () => {
    it('replaces the hash, forces a change, and revokes every session', async () => {
        ...
        // Two live sessions, so "revoked" is a number that could have been wrong.
    ...
    it('emitted an audit row that names neither the old nor the new hash', async () => {
        ...
        expect(JSON.parse(serialised).sessionsRevoked).toBe(2)
    })
})
...
describe('session:revoke-all', () => {
    it('revokes every live session on the instance', async () => {
        ...
        const remaining = await dataSource.getRepository(Session).count({ where: { revokedDate: null as never } })
...
describe('doctor', () => {
    ...
    beforeAll(async () => {
        ...
        const realCredential = await dataSource
            .getRepository(Credential)
            .save(
                dataSource
                    .getRepository(Credential)
                    .create({ name: 'openai', credentialName: 'openAIApi', encryptedData: 'x', workspaceId: workspace.id })
            )
        ...
        await dataSource.getRepository(ChatFlow).save(
            dataSource.getRepository(ChatFlow).create({
                ...
                flowData: JSON.stringify({
                    nodes: [{ id: 'n1', data: { credential: realCredential.id, inputs: {} } }]
                })
            })
        )
    ...
    it('detects the deliberately orphaned tenant key and names the row', async () => {
        ...
        // And the tables that have NOT been denormalised are reported, not silently passed.
        expect(check?.details.join('\n')).toContain('tables without an organizationId column')
    })
